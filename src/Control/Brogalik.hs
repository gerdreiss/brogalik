...
generateBrogalik size =
  Brogalik { brogalikSize = size
           , brogalikRooms = array roomsIndexRange
               $ zip (range roomsIndexRange) rooms
...
brogalikMove direction rogalik =
  rogalik { brogalikPlayer = playerMove direction player }
  where
    player = brogalikPlayer rogalik
...
playerMove direction player =
  ...
  where
    newPos (Pos x y) (PosDelta changeX changeY) =
