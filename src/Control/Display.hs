...
fillDisplay pixel display = _fillRect rect pixel display
  where
    rect = Rect (Pos 0 0) (displaySize display)
...
renderDisplay (Display size@(Size width height) pixels) =
  unlines [[pixels ! Pos x y | x <- [0 .. width - 1]] | y <- [0 .. height - 1]]
...
displayPlayer brogalik = displayPixel playerScreenPos '@'
  where
    ...
    Rect playerRoomPos _ = roomRect
      (brogalikRooms brogalik ! playerRoom player)

    player = brogalikPlayer brogalik

-- | Display the rooms within the Brogalik
displayRooms :: Brogalik -> Display -> Display
displayRooms rogalik display = foldl' (flip displayRoom) display roomList
  where
...
_fillRect rect pixel display =
  display { displayPixels = pixels
              // do
...
_drawVLine (Pos x y) height = _fillRect
  $ Rect (Pos x y) (Size 1 (height - y + 1))

_drawHLine :: Pos -> Width -> Pixel -> Display -> Display
_drawHLine (Pos x y) width = _fillRect
  $ Rect (Pos x y) (Size (width - x + 1) 1)

_drawRect :: Rect -> Pixel -> Display -> Display
_drawRect rect pixel = _drawHLine (Pos x y) width pixel
